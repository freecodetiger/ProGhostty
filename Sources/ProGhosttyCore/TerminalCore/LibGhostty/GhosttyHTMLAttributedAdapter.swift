...
public final class GhosttyHTMLAttributedAdapter {
  ...
  public func attributedString(fromHTML html: String, isFocused: Bool) throws -> NSAttributedString {
    ...
    let normalized = Self.normalizedHTMLPaletteColors(in: html)
  ...
  private func append(
    ...
    var foreground = style.invisible ? backgroundBase : foregroundBase
    var background = backgroundBase

    if style.inverse {
      swap(&foreground, &background)
    }
    if style.faint {
      foreground = blended(foreground, toward: palette.background, amount: 0.52)
    }
    if !isFocused {
      foreground = blended(foreground, toward: palette.background, amount: palette.inactiveForegroundBlend)
    }
  ...
  private func blended(_ color: NSColor, toward background: NSColor, amount: CGFloat) -> NSColor {
    let rgb = color.usingColorSpace(.deviceRGB) ?? color
    let bg = background.usingColorSpace(.deviceRGB) ?? background
    return NSColor(
      calibratedRed: rgb.redComponent + (bg.redComponent - rgb.redComponent) * amount,
      green: rgb.greenComponent + (bg.greenComponent - rgb.greenComponent) * amount,
      blue: rgb.blueComponent + (bg.blueComponent - rgb.blueComponent) * amount,
      alpha: rgb.alphaComponent
    )
  }

  public static func normalizedHTMLPaletteColors(in html: String) -> String {
