...
/// A full key event routed through libghostty-vt's key encoder, mirroring
/// Ghostty's macOS input path. `keyCode` is the raw macOS virtual keycode
/// (`NSEvent.keyCode`); the shim maps it to a physical Ghostty key.
public struct TerminalKeyEvent: Sendable, Equatable {
  public var keyCode: UInt16
  public var modifiers: TerminalKeyModifiers
  /// The text generated by the key, or nil for pure function/navigation keys.
...
/// Modifier bitmask matching libghostty-vt's `GhosttyMods` bits.
public struct TerminalKeyModifiers: OptionSet, Sendable, Equatable {
  ...
  public static let alt = TerminalKeyModifiers(rawValue: 1 << 2)
  public static let superKey = TerminalKeyModifiers(rawValue: 1 << 3)
  public static let capsLock = TerminalKeyModifiers(rawValue: 1 << 4)
  public static let numLock = TerminalKeyModifiers(rawValue: 1 << 5)
  public static let shiftRight = TerminalKeyModifiers(rawValue: 1 << 6)
  public static let controlRight = TerminalKeyModifiers(rawValue: 1 << 7)
  public static let altRight = TerminalKeyModifiers(rawValue: 1 << 8)
  public static let superRight = TerminalKeyModifiers(rawValue: 1 << 9)
}
...
public final class GhosttyVTBridge {
  ...
  public func encodedKey(_ event: TerminalKeyEvent) throws -> Data {
    try locked {
      guard let handle else { return Data() }
      var pointer: UnsafeMutablePointer<UInt8>?
      var length = 0
      let utf8 = Data((event.text ?? "").utf8)
      let result = utf8.withUnsafeBytes { bytes -> Int32 in
        let utf8Ptr: UnsafePointer<UInt8>? = bytes.baseAddress.map { $0.assumingMemoryBound(to: UInt8.self) }
        return proghostty_vt_encode_key(
          handle,
          event.keyCode,
