...
struct TerminalTUIScrollQuantizerTests {
  @Test func preciseDeltasAccumulateByCellHeight() {
    ...
    #expect(quantizer.consume(delta: 3, precise: true, cellHeight: 16) == 0)
    #expect(quantizer.pendingDelta == 6)
    #expect(quantizer.consume(delta: 5, precise: true, cellHeight: 16) == 1)
  ...
  @Test func discreteWheelUsesGhosttyDefaultThreeLineMultiplier() {
    ...
    #expect(quantizer.consume(delta: 0.1, precise: false, cellHeight: 16) == 3)
    #expect(quantizer.consume(delta: -0.1, precise: false, cellHeight: 16) == -3)
    #expect(quantizer.pendingDelta == 0)
  }
  ...
  @Test func preciseRemainderSurvivesWholeUnitEmission() {
    ...
    #expect(quantizer.consume(delta: 10, precise: true, cellHeight: 16) == 1)
    #expect(quantizer.pendingDelta == 4)
    #expect(quantizer.consume(delta: 6, precise: true, cellHeight: 16) == 1)
    #expect(quantizer.pendingDelta == 0)
  }

  @Test func resetClearsPendingInput() {
    var quantizer = TerminalTUIScrollQuantizer()
    _ = quantizer.consume(delta: 3, precise: true, cellHeight: 16)
